(function() {
  ...
  function updateDashboardWithData(data) {
    ...
    const replacements = [
      ...
      ['MRR 1.16억', 'MRR ' + formatEok(data.revenue)],
      ...
      ['2,488명', formatNum(data.payingUsers) + '명'],
      ['2,488', formatNum(data.payingUsers)],
      ['2,453명', formatNum(data.payingUsers) + '명'],
      ['2,453', formatNum(data.payingUsers)],

      // Additional MAU patterns
      ['5.8만명', formatMan(data.mau) + '명'],
      ['5.8만', formatMan(data.mau)],
      ['6.9만명', formatMan(data.mau) + '명'],
      ['6.9만', formatMan(data.mau)],

      // 광고 클릭 수 (퍼널 관련)
      ['66,093', formatNum(funnelAdClicks)],

      // Free users
      ['63,090명', formatNum(freeUsers) + '명'],
      ...
      // Stickiness patterns
      ['4.13%', formatPercent(data.stickiness)],
      ...
      // Paid D1 Retention patterns (결제자 D1 리텐션)
      ['결제자: 11.6%', '결제자: ' + paidD1Retention.toFixed(1) + '%'],
      ['결제 유저 D1 11.6%', '결제 유저 D1 ' + paidD1Retention.toFixed(1) + '%'],
      ['D1 11.6%', 'D1 ' + paidD1Retention.toFixed(1) + '%'],
      ['11.6%', paidD1Retention.toFixed(1) + '%'],

      // Paid User Engagement patterns (결제자 참여도)
      ['2.8배로', paidUserEngagement.toFixed(1) + '배로'],
      ['2.8배 (PMF', paidUserEngagement.toFixed(1) + '배 (PMF'],
      ['참여도 2.8배', '참여도 ' + paidUserEngagement.toFixed(1) + '배'],
      ['의 2.8배', '의 ' + paidUserEngagement.toFixed(1) + '배'],
      ['= 2.8배', '= ' + paidUserEngagement.toFixed(1) + '배'],

      // Additional MRR patterns (억원 단위)
      ['1.08억원', formatEok(data.revenue) + '원'],
      ['1.08억', formatEok(data.revenue)],
      ['0.88억원', formatEok(data.revenue) + '원'],
      ['0.88억', formatEok(data.revenue)],
      ['월 8,663만원', '월 ' + formatManWonUnit(data.revenue)],
      ['8,663만원 매출', formatManWonUnit(data.revenue) + ' 매출'],
      ['MRR 8,663만원', 'MRR ' + formatManWonUnit(data.revenue)],
      ['8,663만원', formatManWonUnit(data.revenue)],
      ['8,663만', formatManWon(data.revenue)],
      ['ARR 10.4억', 'ARR ' + formatEokShort(data.arr)],

      // Additional ARPPU patterns
      ['35,924원+', formatWon(data.arppu) + '+'],
      ['35,924원', formatWon(data.arppu)],
      ...
      ['6.9만명', formatMan(data.mau) + '명'],
      ['6.9만', formatMan(data.mau)],
      ['MAU 6.6만', 'MAU ' + formatMan(data.mau)],
      ['MAU 6.9만', 'MAU ' + formatMan(data.mau)],

      // D1 Retention text patterns
      ['D1 4.4%', 'D1 ' + formatPercent(data.d1Retention)],
      ['D1 4.55%', 'D1 ' + formatPercent(data.d1Retention)],
      ['D1 4.64%', 'D1 ' + formatPercent(data.d1Retention)],
      ['D1 리텐션 4.4%', 'D1 리텐션 ' + formatPercent(data.d1Retention)],
      ['4.4%는 업계', formatPercent(data.d1Retention) + '는 업계'],
      ['(4.4%)의', '(' + formatPercent(data.d1Retention) + ')의'],
      ['비결제자(4.4%)', '비결제자(' + formatPercent(data.d1Retention) + ')'],

      // MRR/Revenue in context
      ['MRR 0.88억', 'MRR ' + formatEok(data.revenue)],
      ['MRR 1.08억', 'MRR ' + formatEok(data.revenue)],
      ['월 1.08억', '월 ' + formatEok(data.revenue)]
    ];
