...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Query MAU from free_saju_results (unique phone numbers)
    const [mauRows] = await bigquery.query({
      query: `SELECT COUNT(DISTINCT phone) as mau
              FROM (
                SELECT phone FROM \`${projectId}.supabase_sync.free_saju_results\`
                WHERE DATE(created_at) BETWEEN '${startDash}' AND '${endDash}' AND phone IS NOT NULL
                UNION ALL
                SELECT phone FROM \`${projectId}.supabase_sync.free_love_saju_results\`
                WHERE DATE(created_at) BETWEEN '${startDash}' AND '${endDash}' AND phone IS NOT NULL
                UNION ALL
                SELECT phone FROM \`${projectId}.supabase_sync.free_marriage_saju_results\`
                WHERE DATE(created_at) BETWEEN '${startDash}' AND '${endDash}' AND phone IS NOT NULL
              )`
    });
    ...
    // D1 Retention (based on free_saju_results - users who came back next day)
    const [d1Rows] = await bigquery.query({
      query: `WITH user_first AS (
              ...
              d1 AS (
                ...
                FROM user_first f
                JOIN \`${projectId}.supabase_sync.free_saju_results\` e
                  ON f.phone = e.phone
                  AND DATE(e.created_at) = DATE_ADD(f.first_date, INTERVAL 1 DAY)
              )
              SELECT ROUND(COUNT(DISTINCT d.phone) / NULLIF(COUNT(DISTINCT f.phone), 0) * 100, 2) as d1
              FROM user_first f LEFT JOIN d1 d ON f.phone = d.phone`
    });
    if (d1Rows[0]) data.d1Retention = parseFloat(d1Rows[0].d1) || 0;

    // Stickiness (DAU/MAU based on free service usage)
    const [stickinessRows] = await bigquery.query({
      query: `WITH daily AS (
                SELECT DATE(created_at) as date, COUNT(DISTINCT phone) as dau
                FROM \`${projectId}.supabase_sync.free_saju_results\`
                WHERE DATE(created_at) BETWEEN '${startDash}' AND '${endDash}' AND phone IS NOT NULL
                GROUP BY date
              )
